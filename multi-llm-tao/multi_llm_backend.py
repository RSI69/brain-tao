...
# In route_models(): skip stale
all_uids = [
    uid for uid in metagraph.uids.tolist()
    if metagraph.total_stake[uid] > MIN_STAKE
    and metagraph.axons[uid].is_serving
    and (timestamp_now() - last_seen[uid]) < HEALTH_TTL

if __name__ == '__main__':
    async def fuzz_test():
        dummy = ChatRequest(
            prompt="What is AI?", session_id="test", wallet_address="0x0", signature="0x0",
            estimated_cost=0.1
        )
        try:
            r = await chat(dummy, None)
            print(r)
        except Exception as e:
            print("Fuzz test failed:", e)
    asyncio.run(fuzz_test())
...
def route_models(prompt: str, quality: str, judge: bool, synth: bool, coders: bool, num_generators: int = 3, num_coders: int = 2) -> Dict[str, List[int]]:
    import bittensor
    ...
    quality_map = {
        "ultra": all_uids[:20],
        "high": all_uids[:50],
        "medium": all_uids[50:100],
        "low": all_uids[-50:]
    }

    pool = quality_map.get(quality.lower(), all_uids[:20])
    seen = set()
    deduped = []
    for uid in pool:
    ...
    if len(prompt.split()) > 800 or len(prompt) > 4096:
        deduped.insert(0, LONG_SLOT)  # ❌ deduped not defined yet


    num_generators = min(num_generators, len(deduped))
    num_coders = min(num_coders, len(deduped) - num_generators)

    return {
        "generators": deduped[:num_generators],
        "coders": deduped[num_generators:num_generators + num_coders] if coders else [],
        "judge": deduped[num_generators + num_coders:num_generators + num_coders + 2] if judge else [],
        "synth": deduped[num_generators + num_coders + 2:num_generators + num_coders + 4] if synth else []
    }
...
class ChatRequest(BaseModel):
    ...
    quality_mode: Optional[str] = "ultra"
    retries: Optional[int] = 1
    use_judge: Optional[bool] = True
    use_synth: Optional[bool] = True
    include_coders: Optional[bool] = True
...
async def stream(req: ChatRequest, request: Request):
    async def stream_response():
        ...
        try:
            models = route_models(req.prompt)
            outputs = await asyncio.gather(*[generate(m, full_prompt, MAX_TOKENS_GEN) for m in models])
...
async def cost_estimate(req: ChatRequest):
    ...
    gen_cost = 3 * avg("generator")
    coder_cost = 2 * avg("coder") if req.include_coders else 0
    judge_cost = avg("judge") if req.use_judge else 0
    synth_cost = avg("synth") if req.use_synth else 0
    ...
    return {
        ...
        "roles_used": {
            "generators": req.include_coders or True,
            "coders": req.include_coders,
            "judge": req.use_judge,
            "synth": req.use_synth
        }
    }
